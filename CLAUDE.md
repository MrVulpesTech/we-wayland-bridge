...
- [!] Session 4 (GNOME extension consumer): pipeline works end-to-end, but the **first live test crashed gnome-shell** (SIGSEGV in libgstpipewire on pipeline teardown = session end = the apparent "freeze"; plus a grey-screen login loop from blocking the main thread at login with no producer). **All three failure modes now fixed AND re-verified in a nested session** (no-producer login no longer hangs; producer-disconnect + clean disable leave the shell alive, no SIGSEGV). Hardening: deferred fail-open init, guarded enable(), kill-switch (`~/.config/wwb/disabled`, `WWB_FORCE` bypass), clean teardown (NULL+wait), off-screen upload guard. Full writeup `docs/40_bridge/40.04_incident_2026-06-12_freeze.md`. **A careful live retry is now reasonable but is the operator's call** (40.03 runbook; visual needs live since `mutter-devkit` viewer isn't packaged). Original end-to-end note below:
